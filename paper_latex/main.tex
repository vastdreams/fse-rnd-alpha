...
\begin{itemize}[leftmargin=*]
    \item The annual non-overlapping premium averages \AnnualMeanPremium\% ($t = \AnnualTStat$, $p = \AnnualPValue$). While the annual time-series test does not reach conventional significance due to high year-to-year variance, monthly factor spanning provides statistically significant evidence (FF5 $\alpha = 4.37\%$, $p < 0.01$) and Fama-MacBeth regressions provide directionally consistent cross-sectional evidence ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars).
