...
\noindent\textbf{Results.} The high-minus-low R\&D factor (\textbf{HML\_RD}) averages \textbf{\AnnualMeanPremium\%} per year ($t = \AnnualTStat$, $p = \AnnualPValue$; Newey-West \citep{newey_west_1987}), positive in \AnnualPositiveYears{}/\AnnualNYears{} years (\AnnualWinRatePct\% win rate). The investable \textbf{RD20 strategy} delivers \textbf{\NetPremiumAfterCostsPct\%} annual excess return versus SPY after transaction costs (\BacktestPeriodLabel{} backtest, N=\BacktestNYears), retaining \PremiumCaptureRatePct\% of the gross premium.
...
\end{itemize}
\textbf{Interpretation.} The point estimate (\AnnualMeanPremium\% annually) is economically meaningful---comparable in magnitude to many established factor premiums---though not statistically significant at the 5\% level ($p = \AnnualPValue$). With a 24-year sample and high year-to-year variance, wide confidence intervals are expected. The premium is positive in \AnnualWinRatePct\% of years.
\end{tcolorbox}
...
In a large-cap U.S.\ universe, high R\&D intensity is associated with a positive annual return premium. The within-universe HML\_RD spread (\AnnualMeanPremium\% annually) is economically meaningful though not statistically significant at conventional levels with 24 observations. The investable RD20 strategy delivers substantial excess returns versus SPY (\NetPremiumAfterCostsPct\% net annually over \BacktestPeriodLabel) and appears largely capturable under a calibrated transaction-cost model.

The primary findings are:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Existence:} The annual non-overlapping HML premium averages \AnnualMeanPremium\% ($t = \AnnualTStat$, $p = \AnnualPValue$). While not statistically significant at the 5\% level with 24 observations, the point estimate is economically meaningful---comparable to many established factor premiums.
