...
\noindent\textbf{Results.} The high-minus-low R\&D factor (\textbf{HML\_RD}) averages \textbf{\AnnualMeanPremium\%} per year ($t = \AnnualTStat$, $p = \AnnualPValue$; Newey-West \citep{newey_west_1987}), positive in \AnnualPositiveYears{}/\AnnualNYears{} years (\AnnualWinRatePct\% win rate). The annual test does not reach conventional significance due to high variance, but monthly factor spanning tests confirm a statistically significant premium: FF5 $\alpha = 4.37\%$ ($t = 3.01$, $p < 0.01$). The investable \textbf{RD20 strategy} delivers \textbf{\NetPremiumAfterCostsPct\%} annual excess return versus SPY after transaction costs (\BacktestPeriodLabel{} backtest, N=\BacktestNYears), retaining \PremiumCaptureRatePct\% of the gross premium.
...
\begin{itemize}[leftmargin=*]
    \item The annual non-overlapping premium averages \AnnualMeanPremium\% ($t = \AnnualTStat$, $p = \AnnualPValue$); while not significant at 5\% in the annual time-series test due to high year-to-year variance, the FF5 monthly spanning alpha (4.37\%, $p < 0.01$) provides statistically significant evidence of a distinct premium.
...
In a large-cap U.S.\ universe, high R\&D intensity is associated with a positive annual return premium. The within-universe HML\_RD spread (\AnnualMeanPremium\% annually over \AnnualNYears{} years) is economically meaningful. While the annual time-series test does not reach conventional significance ($p = \AnnualPValue$) due to high year-to-year variance, the monthly factor spanning tests provide strong statistical evidence: the FF5 alpha is 4.37\% annually ($t = 3.01$, $p < 0.01$, 360 observations). The investable RD20 strategy delivers substantial excess returns versus SPY (\NetPremiumAfterCostsPct\% net annually over \BacktestPeriodLabel) and appears largely capturable under a calibrated transaction-cost model.

The primary findings are:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Existence:} The annual non-overlapping HML premium averages \AnnualMeanPremium\% over \AnnualNYears{} years ($t = \AnnualTStat$, $p = \AnnualPValue$). While the annual time-series test does not reach conventional significance due to high variance (particularly from the dot-com era), the FF5 monthly spanning test is highly significant: $\alpha = 4.37\%$ annually ($t = 3.01$, $p < 0.01$). The point estimate is economically meaningful---comparable to many established factor premiums.
