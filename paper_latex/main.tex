...
\end{equation}
where $P$ is price and $D$ is dividends paid during the period. Tier-1 uses closing-price returns as a practical proxy and does not add dividends separately; Tier-2 (when available) uses CRSP-style total returns with authoritative delisting treatment.

\textbf{Tier-1 implementation note (this snapshot):} The Tier-1 daily price endpoint used here provides closing prices but does \emph{not} provide a dividend-adjusted ``adjusted close'' series. Accordingly, Tier-1 returns in this snapshot are computed from closing prices (price returns) and we do not add dividends separately. This limitation is explicitly disclosed and motivates Tier-2 (CRSP/Compustat) robustness work when available.
...
Table~\ref{tab:sample} reports sample construction and coverage statistics for the snapshot. Table~\ref{tab:universe_integrity} documents the point-in-time membership enforcement used in this study.

\input{tables/table_sample.tex}

\paragraph{Universe integrity.} We use official S\&P 500 ``Date added'' information (sourced from Wikipedia's S\&P 500 list, which tracks S\&P Dow Jones Indices announcements) to enforce point-in-time membership at each July~1 formation date. Table~\ref{tab:universe_integrity} summarizes the result: average constituents per year range from 189 (2001) to 487 (2024), reflecting that our universe includes only companies that had been added to the index by each formation date. \textbf{Limitation:} Historical removals are not tracked in our Tier-1 source, so companies that were removed from the S\&P 500 but are still active appear in all years after their add date. This biases toward higher survival rates than true point-in-time membership and should be considered when interpreting results.
...
\begin{tcolorbox}[title=Exit Handling in Tier-1 Data (Why This Is Not CRSP dlret), colback=yellow!5, colframe=yellow!40, breakable]
...
\textbf{Bottom line:} Exit handling is explicit and bias-aware, but delisting-return uncertainty remains a known limitation without CRSP/Compustat-grade data.
\end{tcolorbox}
...
\begin{tcolorbox}[title=Hard truth: Sector component is material, colback=yellow!5, colframe=yellow!50]
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Net premium (benchmark-relative):} After costs, the strategy delivers \NetPremiumAfterCostsPct\% annual net premium versus SPY (S\&P 500 price-return proxy).
\end{itemize}
...
\end{itemize}
Citation metadata is provided in \path{CITATION.cff} in the repository and on the platform's ``How to Cite'' section.
