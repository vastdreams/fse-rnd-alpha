...
\noindent\textbf{Results.} The high-minus-low R\&D factor (\textbf{HML\_RD}) averages \textbf{\AnnualMeanPremium\%} per year ($t = \AnnualTStat$, $p = \AnnualPValue$; Newey-West \citep{newey_west_1987}), positive in \AnnualPositiveYears{}/\AnnualNYears{} years (\AnnualWinRatePct\% win rate). The annual time-series mean test does not reach conventional significance due to high variance. However, monthly cross-sectional evidence is statistically significant: Fama-MacBeth regressions \citep{fama_macbeth_1973} show R\&D intensity predicts returns after controlling for size and book-to-market ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars), and factor spanning tests confirm a distinct premium (FF5 $\alpha = 4.37\%$, $p < 0.01$). The investable \textbf{RD20 strategy} delivers \textbf{\NetPremiumAfterCostsPct\%} annual excess return versus SPY after transaction costs (\BacktestPeriodLabel{} backtest, N=\BacktestNYears), retaining \PremiumCaptureRatePct\% of the gross premium.
...
\end{itemize}
\textbf{Key result:} R\&D intensity is a statistically significant predictor of next-month returns ($t = \FamaMacBethRDTStat$, $p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars{}) after controlling for size and value.
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*]
    \item The annual non-overlapping premium averages \AnnualMeanPremium\% ($t = \AnnualTStat$, $p = \AnnualPValue$). While the annual time-series test does not reach conventional significance due to high year-to-year variance, cross-sectional and monthly evidence is statistically significant: Fama-MacBeth regressions ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars) and FF5 monthly spanning ($\alpha = 4.37\%$, $p < 0.01$).
...
In a large-cap U.S.\ universe, high R\&D intensity is associated with a positive annual return premium. The within-universe HML\_RD spread (\AnnualMeanPremium\% annually over \AnnualNYears{} years) is economically meaningful. While the annual time-series test does not reach conventional significance ($p = \AnnualPValue$) due to high year-to-year variance, monthly cross-sectional and factor evidence provides statistically significant support: Fama-MacBeth regressions show R\&D intensity predicts returns ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars), and the FF5 alpha is 4.37\% annually ($p < 0.01$). The investable RD20 strategy delivers substantial excess returns versus SPY (\NetPremiumAfterCostsPct\% net annually over \BacktestPeriodLabel) and appears largely capturable under a calibrated transaction-cost model.

The primary findings are:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Existence:} The annual non-overlapping HML premium averages \AnnualMeanPremium\% over \AnnualNYears{} years ($t = \AnnualTStat$, $p = \AnnualPValue$). While the annual time-series test does not reach conventional significance due to high variance, cross-sectional and monthly evidence is statistically significant: Fama-MacBeth regressions ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars, \FamaMacBethNMonths{} months) and FF5 spanning ($\alpha = 4.37\%$, $p < 0.01$). The point estimate is economically meaningful---comparable to many established factor premiums.
