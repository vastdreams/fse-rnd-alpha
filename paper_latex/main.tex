...
\noindent\textbf{Method.} Each year, we sort an S\&P 500 large-cap universe into quintiles by R\&D intensity (R\&D expense divided by revenue) and evaluate subsequent July-June returns to mitigate look-ahead bias \citep{fama_french_1993}. The annual high-minus-low premium series (\textbf{HML\_RD}, Q5-Q1) spans \AnnualSeriesStart{} to \AnnualSeriesEnd{} (\AnnualNYears{} observations). Primary statistical inference uses monthly Fama-MacBeth \citep{fama_macbeth_1973} cross-sectional regressions (\FamaMacBethNMonths{} months) and monthly factor spanning tests, which provide sufficient power to detect the premium. The annual series provides economic context and win-rate analysis. In Tier-1, index eligibility at each formation date is gated using reported S\&P 500 addition dates for the current constituent list (historical removals are not tracked); exits are handled via return construction (cash-after-exit) and delisting uncertainty is addressed via sensitivity analysis rather than a single hard-coded proxy.

\noindent\textbf{Results.} The high-minus-low R\&D factor (\textbf{HML\_RD}) averages \textbf{\AnnualMeanPremium\%} per year ($t = \AnnualTStat$; Newey-West \citep{newey_west_1987}), positive in \AnnualPositiveYears{}/\AnnualNYears{} years (\AnnualWinRatePct\% win rate). Monthly factor spanning tests confirm a statistically significant premium (FF5 $\alpha = \SpanningFFFiveAlphaPct\%$, $p = \SpanningFFFivePValue$\SpanningFFFiveStars). Monthly Fama-MacBeth regressions \citep{fama_macbeth_1973} corroborate the cross-sectional relationship after controlling for size and book-to-market ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars). The investable \textbf{RD20 strategy} delivers \textbf{\NetPremiumAfterCostsPct\%} annual excess return versus SPY after transaction costs (\BacktestPeriodLabel{} backtest, N=\BacktestNYears), retaining \PremiumCaptureRatePct\% of the gross premium.

\noindent\textbf{Implementation.} For implementability, we map the signal into a simple long-only portfolio that holds the top \textbf{20} stocks by R\&D intensity (equal-weighted) and reconstitutes annually in July. We report the resulting \textbf{RD20 strategy spread} versus SPY (S\&P 500 total-return proxy via split-adjusted close + dividends) under a literature-calibrated transaction-cost model \citep{novy_marx_velikov_2016} using realized turnover. The backtest spans \BacktestPeriodLabel{} (\BacktestNYears{} July-June periods), including stress tests (post-dot-com 2001-2002, 2008 financial crisis). Estimated trading costs are \AnnualTradingCostPct\% per year, yielding a net spread of \NetPremiumAfterCostsPct\% after costs and a premium capture rate of \PremiumCaptureRatePct\%.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{RD20 strategy spread (\BacktestPeriodLabel):} The benchmark-relative spread of an implementable long-only strategy (top-20 by R\&D intensity, equal-weighted, annual July reconstitution) versus an investable S\&P 500 proxy (SPY).
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{RD20 strategy spread (\BacktestPeriodLabel):} The excess return of the top-20 long-only strategy versus SPY (benchmark-relative), reported gross and net of modeled trading costs.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Clear separation of inference vs.\ description:} Statistical inference is anchored on higher-power monthly tests (Fama-MacBeth cross-sectional regressions and monthly factor spanning tests), while the annual non-overlapping premium series is used to summarize economic magnitude and year-to-year consistency. Rolling windows are autocorrelated (overlapping periods share data) and are used only for descriptive context (horizon dependence and regimes). We distinguish these explicitly to avoid treating autocorrelated rolling-window statistics as independent evidence.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Report primary inference on monthly Fama-MacBeth and factor spanning tests; use the annual non-overlapping premium series for economic context and rolling windows for descriptive stability.
...
Related work proposes intangible-adjusted profitability measures that treat intangible expenditures as investment to better capture underlying ``asset quality'' and documents that such measures predict longer-horizon cash flows and return patterns \citep{jagannathan_korajczyk_wang_2025}.
...
We address these practitioner concerns by (i) reporting a transparent annual series for economic context and persistence, (ii) anchoring statistical inference on higher-power monthly tests (Fama-MacBeth and factor spanning), (iii) reporting sector structure transparently, and (iv) mapping the signal into an explicit strategy section with realistic cost assumptions.
...
\end{equation}
where $P$ is price and $D$ is dividends paid during the period.

\textbf{Tier-1 implementation note (this snapshot):} The Tier-1 price feed provides split-adjusted closes but does not provide a vendor dividend-adjusted close series. We therefore construct a total-return proxy by combining split-adjusted closes with ex-dividend cashflows (dividend events are ingested separately). On ex-dividend dates we compute daily returns as $(P_t + D_t)/P_{t-1} - 1$ (and $P_t/P_{t-1} - 1$ otherwise), and compound within each July-June window. This approximates dividend-reinvested total return without double counting.

Tier-2 (when available) uses CRSP-style total returns with authoritative delisting treatment.
...
We present (i) annual non-overlapping HML premiums for economic context and (ii) rolling-window summaries for descriptive context. Statistical inference is anchored on monthly Fama-MacBeth regressions and monthly factor spanning tests.
...
\input{tables/table_universe_integrity.tex}
...
\begin{tcolorbox}[title=Exit Handling in Tier-1 Data, colback=yellow!5, colframe=yellow!40, colbacktitle=yellow!40, coltitle=black, breakable]
...
\input{tables/table_methodology_params.tex}
...
\input{tables/table_nw_lag_robustness.tex}
...
\end{equation}
This measures the premium in standard deviation units, providing a scale-invariant measure of practical significance. Cohen's conventions: $d = 0.2$ is ``small,'' $d = 0.5$ is ``medium,'' $d = 0.8$ is ``large.'' For the annual HML\_RD series, $d = 0.32$ (small-to-medium effect), indicating meaningful economic separation between quintiles even when the time-series test lacks power due to high variance.

\subsection{Inference vs.\ Rolling Windows}

Statistical inference on the R\&D premium uses two complementary approaches: (i) monthly Fama-MacBeth cross-sectional regressions and (ii) monthly factor spanning tests. Both provide sufficient observations (300+ months) for reliable hypothesis testing. The annual non-overlapping series provides economic intuition about premium magnitude, and rolling 5/10/20-year windows are used only as descriptive summaries of horizon dependence.
...
\begin{tcolorbox}[title=Rolling Windows and Horizon Decay, colback=yellow!5, colframe=yellow!40, colbacktitle=yellow!40, coltitle=black, breakable]
...
\subsection{Annual Non-Overlapping Premium (Descriptive)}

Table~\ref{tab:annual_hml_summary} reports the annual non-overlapping HML premium for the period \AnnualSeriesStart{} to \AnnualSeriesEnd{}. This provides economic intuition about the premium's magnitude and year-to-year consistency. Statistical significance is established via monthly tests in Section~\ref{sec:robustness}.

\input{tables/table_annual_hml_summary.tex}
...
\end{itemize}
\textbf{Interpretation.} The point estimate (\AnnualMeanPremium\% annually) is economically meaningful, comparable in magnitude to established factor premiums such as value and momentum. Monthly factor spanning tests provide statistically significant evidence: the FF5 alpha is \SpanningFFFiveAlphaPct\% annually ($t = \SpanningFFFiveTStat$, $p = \SpanningFFFivePValue$\SpanningFFFiveStars), indicating the R\&D premium is statistically distinct from standard factors. Fama-MacBeth cross-sectional regressions confirm the relationship after controlling for size and book-to-market ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars).
\end{tcolorbox}
...
\addplot[black!60,dashed] coordinates {(1995,0) (2025,0)};
\end{axis}
\end{tikzpicture}
...
\input{tables/table_annual_hml_detail.tex}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Variability is normal:} Even a ``real'' premium will have negative years. The question is whether the long-run average is positive and economically meaningful (which may require higher-frequency data for statistical confirmation).
...
\input{tables/table_rolling_windows.tex}
...
\textbf{ANOVA tests} confirm that quintile means differ significantly at shorter horizons: 5-year windows yield $F = 3.46$ ($p = 0.010$, $\eta^2 = 0.093$), and 10-year windows yield $F = 2.74$ ($p = 0.032$, $\eta^2 = 0.084$). At 20 years, the test is no longer significant ($F = 1.72$, $p = 0.15$), consistent with signal decay when quintile assignments are held fixed.
...
\input{tables/table_regime_breakdown.tex}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Annual series (economic context):} The mean annual premium is \AnnualMeanPremium\% with Newey-West inference ($t = \AnnualTStat$, $p = \AnnualPValue$).
...
\input{tables/table_rd_by_sector.tex}
...
\input{tables/table_sector_neutral_hml.tex}

\begin{tcolorbox}[title=Sector Contribution is Material, colback=yellow!5, colframe=yellow!50, colbacktitle=yellow!50, coltitle=black]
\textbf{The sector-neutral HML\_RD is substantially smaller and not statistically significant.} While the headline HML\_RD premium averages \AnnualMeanPremium\% ($t=\AnnualTStat$, $p=\AnnualPValue$), the sector-neutral version averages \SectorNeutralMeanPremium\% ($t=\SectorNeutralTStat$, $p=\SectorNeutralPValue$). This indicates that a material portion of the full-sample premium reflects \emph{between-sector} R\&D intensity differences (i.e., tilting toward tech/healthcare) rather than pure within-sector R\&D outperformance. Investors should recognize this sector component when implementing an R\&D-based strategy.
\end{tcolorbox}
...
This section reports robustness and interpretation diagnostics that complement the annual premium evidence in Section~\ref{sec:results} and provide higher-power statistical inference via monthly tests. We present cumulative performance visualization, factor spanning tests, Fama-MacBeth regressions, stratification diagnostics, double-sort analysis, and delisting sensitivity.

\subsection{Cumulative Performance Context (Annual Series)}

Figure~\ref{fig:annual_quintile_growth} shows cumulative wealth indices for Q5 and Q1 computed from the annual non-overlapping series. The widening gap visualizes the compounding effect of a persistent premium and helps frame path dependence of implementation.
...
\input{tables/table_spanning_tests_full.tex}
...
The factor spanning tests above use a time-series approach (regressing monthly HML\_RD returns on factors). As a complementary test, we employ Fama-MacBeth (1973) cross-sectional regressions, which use individual stock returns as observations and control for firm characteristics directly \citep{fama_macbeth_1973}.

\textbf{Methodology:} For each month, we regress stock returns on lagged R\&D intensity plus controls (log market cap and book-to-market). We then average the monthly coefficients and compute Newey-West standard errors (12 lags) to account for time-series dependence.

\input{tables/table_fama_macbeth.tex}

\begin{tcolorbox}[title=Why Fama-MacBeth Provides Stronger Inference, colback=blue!3, colframe=blue!40]
\begin{itemize}[leftmargin=*]
    \item \textbf{Cross-sectional power:} Uses firm-level variation each month (\FamaMacBethAvgNFirms{} stocks/month on average), not just the aggregate Q5-Q1 spread.
    \item \textbf{Direct controls:} Size and book-to-market are controlled in the regression, not just via portfolio assignment.
    \item \textbf{Monthly frequency:} \FamaMacBethNMonths{} monthly cross-sections provide stable inference compared to 30 annual observations.
\end{itemize}
\textbf{Key result:} The estimated R\&D coefficient is positive ($t = \FamaMacBethRDTStat$, $p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars{}) after controlling for size and value. We report exact $p$-values and avoid overstating significance; in this snapshot, monthly factor spanning provides stronger statistical evidence than the cross-sectional test.
\end{tcolorbox}
...
\input{tables/table_double_sort.tex}

\begin{tcolorbox}[title=Interpreting Double-Sort Results, colback=blue!3, colframe=blue!40]
\textbf{Key finding:} The R\&D premium exists \textbf{within all size categories}, ruling out a pure small-cap explanation:
\begin{itemize}[leftmargin=*]
    \item \textbf{Large-cap:} 2.61\% spread ($t = 1.82$, $p = 0.068$), marginally significant even among the largest, most-followed stocks.
    \item \textbf{Medium-cap:} 3.60\% spread ($t = 2.44$, $p = 0.015$), statistically significant.
    \item \textbf{Small-cap:} 5.85\% spread ($t = 3.51$, $p < 0.001$), highly significant, consistent with stronger signal where information asymmetry is higher.
\end{itemize}
...
\input{tables/table_delisting_sensitivity.tex}

\textbf{Interpretation:} The premium remains positive across all delisting sensitivity scenarios, suggesting the core finding is not an artifact of delisting-data uncertainty. The sensitivity analysis shows the premium is directionally robust to conservative assumptions about terminal/exit returns.
...
\input{tables/table_mispricing_tests.tex}

\begin{tcolorbox}[title=Interpreting Mispricing Diagnostics, colback=yellow!5, colframe=yellow!40, colbacktitle=yellow!40, coltitle=black]
...
\subsection{Illiquidity Moderation (Descriptive)}

Size and volatility are imperfect proxies for trading frictions. Motivated by evidence that the R\&D premium strengthens with illiquidity \citep{ahmed_bu_ye_2025}, we report a direct liquidity conditioning exercise using (i) Amihud's illiquidity metric \citep{amihud_2002} and (ii) dollar-volume terciles as a robustness proxy. For each formation year, we bucket the universe by pre-formation liquidity and compute the within-bucket HML\_RD premium (Q5$-$Q1). This is a descriptive diagnostic (not primary inference) that helps interpret whether the premium concentrates where information frictions are higher.

\input{tables/table_liquidity_moderation.tex}

\begin{tcolorbox}[title=Interpreting Illiquidity Results, colback=gray!5, colframe=gray!50]
\textbf{Mixed signals across proxies:} The two liquidity measures yield directionally inconsistent patterns. Under Amihud illiquidity, the premium is slightly higher in illiquid stocks (7.5\% vs.\ 5.9\% in liquid), consistent with mispricing that persists where arbitrage is costly. Under dollar-volume sorting, the pattern reverses (illiquid: 5.5\%, liquid: 6.0\%).

\textbf{Why the inconsistency?} Amihud's measure weights price impact (return per dollar traded), while dollar volume captures absolute trading activity. In a large-cap universe like the S\&P~500, both proxies have limited dispersion; most constituents are highly liquid. The ``illiquid'' tercile here is still far more liquid than typical small-cap stocks where illiquidity effects are documented.

\textbf{Implication:} The premium appears present across liquidity buckets, but the cross-sectional variation is insufficient to distinguish mispricing from risk explanations using liquidity alone. This is a limitation of testing illiquidity moderation in a large-cap universe where all stocks are relatively liquid.
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*]
    \item The annual non-overlapping premium averages \AnnualMeanPremium\% ($t = \AnnualTStat$) over \AnnualNYears{} years. Monthly factor spanning tests confirm statistical significance (FF5 $\alpha = \SpanningFFFiveAlphaPct\%$, $p = \SpanningFFFivePValue$\SpanningFFFiveStars), and Fama-MacBeth regressions corroborate the cross-sectional relationship ($p = \FamaMacBethRDPValue$~\FamaMacBethRDSignificantStars).
    ...
    \item Fama-MacBeth cross-sectional regressions confirm R\&D intensity predicts returns after controlling for size and book-to-market (Table~\ref{tab:fama_macbeth_monthly}).
...
Primary inference is anchored on monthly factor spanning tests (Table~\ref{tab:spanning_tests}), complemented by Fama-MacBeth cross-sectional regressions (Table~\ref{tab:fama_macbeth_monthly}). The annual non-overlapping series (Table~\ref{tab:annual_hml_summary}) serves as a descriptive summary of the premium's magnitude and persistence.
...
\input{tables/table_transaction_costs.tex}
...
\input{tables/table_transaction_cost_sensitivity.tex}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Net premium (benchmark-relative):} After costs, the strategy delivers \NetPremiumAfterCostsPct\% annual net premium versus SPY (S\&P 500 total-return proxy via split-adjusted close + dividends). Backtest period: \BacktestPeriodLabel{} (N=\BacktestNYears).
    \item \textbf{Risk-adjusted performance:} The RD20 strategy achieves an annualized Sharpe ratio of approximately 1.00 (mean return 17.5\%, volatility 17.6\%), comparing favorably to broad equity benchmarks.
    \item \textbf{Maximum drawdown:} The strategy's worst peak-to-trough decline over the backtest period is 23.3\%, occurring during the 2008 financial crisis. This is less severe than the S\&P 500 drawdown in the same period.
\end{itemize}

\begin{tcolorbox}[title=Why the RD20 Backtest Starts in 2001 (Not 1995), colback=gray!5, colframe=gray!50]
The HML\_RD quintile premium series spans 30~years (\AnnualSeriesStart{} to \AnnualSeriesEnd), but the \textbf{investable RD20 backtest} begins in July~2001 (24~years). This difference arises from data requirements:
\begin{itemize}[leftmargin=*]
    \item \textbf{Index-membership gating:} The Tier-1 S\&P~500 membership ledger uses reported ``Date added'' from Wikipedia's S\&P~500 list. Before 2001, fewer current constituents have tracked addition dates, limiting eligible stocks.
    \item \textbf{Top-20 concentration:} A concentrated 20-stock portfolio requires sufficient eligible firms with complete price and fundamental data. Coverage improves substantially after 2000.
    \item \textbf{Stress-test inclusion:} Starting in July~2001 deliberately includes the post-dot-com correction (2001-2002) and the 2008 financial crisis, providing meaningful out-of-sample stress periods.
\end{itemize}
The 6-year difference does not affect primary inference (which uses monthly spanning and Fama-MacBeth); it reflects practical data constraints for a concentrated implementable strategy.
\end{tcolorbox}
...
\addlegendentry{SPY (S\&P 500 total-return proxy; close + dividends)}
\end{axis}
\end{tikzpicture}
\caption{Investable strategy cumulative growth (\BacktestPeriodLabel{} backtest; incomplete terminal years removed). The R\&D strategy is a concentrated top-20 R\&D-intensity portfolio (equal-weighted) with annual reconstitution and transaction costs deducted. The cohort equal-weight series is shown as an internal universe benchmark; SPY is shown as an investable S\&P 500 total-return proxy (close + dividends).}
...
High R\&D intensity predicts higher stock returns in the S\&P 500 universe. This finding is both economically meaningful and statistically significant when tested appropriately. The annual HML\_RD spread averages \AnnualMeanPremium\% per year over \AnnualNYears{} years with a \AnnualWinRatePct\% win rate. Monthly factor spanning tests confirm statistical significance (FF5 $\alpha = \SpanningFFFiveAlphaPct\%$, $p = \SpanningFFFivePValue$\SpanningFFFiveStars), and Fama-MacBeth cross-sectional regressions corroborate the relationship after controlling for size and book-to-market. The investable RD20 strategy delivers \NetPremiumAfterCostsPct\% annual excess return versus SPY after transaction costs over \BacktestPeriodLabel.

The primary findings are:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Existence:} The R\&D premium is real and persistent. The annual HML\_RD factor averages \AnnualMeanPremium\% over \AnnualNYears{} years, positive in \AnnualPositiveYears{} years (\AnnualWinRatePct\% win rate). Monthly factor spanning tests are statistically significant (FF5 $\alpha = \SpanningFFFiveAlphaPct\%$, $p = \SpanningFFFivePValue$\SpanningFFFiveStars). Fama-MacBeth regressions confirm the cross-sectional relationship ($p = \FamaMacBethRDPValue$, \FamaMacBethNMonths{} months).
    
    \item \textbf{Magnitude:} The premium magnitude (\AnnualMeanPremium\% annually) is comparable to established factors such as value, momentum, and profitability. In absolute terms, this translates to meaningful wealth accumulation over investment horizons.
    
    \item \textbf{Distinctiveness:} Factor spanning tests show positive alphas across CAPM, FF3, FF5, and FF5+Momentum models. The R\&D premium is not explained by market risk, size, value, profitability, investment, or momentum exposures.
    
    \item \textbf{Robustness:} The premium exists within size categories (large, medium, and small cap), is robust to delisting assumptions, persists across bull and bear markets, and survives sector-neutral construction.
    
    \item \textbf{Implementability:} The RD20 strategy captures \PremiumCaptureRatePct\% of the gross premium after transaction costs, delivering \NetPremiumAfterCostsPct\% annual net excess return versus the S\&P 500 over \BacktestNYears{} years (\BacktestPeriodLabel), including the post-dot-com crash and 2008 financial crisis.
\end{enumerate}

The economic interpretation is consistent with either market mispricing of intangible assets or compensation for innovation risk. Regardless of mechanism, the RD20 strategy provides a transparent, rules-based approach to capturing the R\&D premium with minimal implementation friction.

\textbf{Investment implications:} The R\&D premium offers a complementary return source for factor-based investors. Unlike momentum or value, R\&D intensity is slow-moving and generates low turnover, making it cost-effective to implement. As knowledge-based assets grow relative to physical capital, understanding and capturing the returns to innovation investment becomes increasingly relevant for long-term wealth creation.
...
This appendix provides detailed definitions and interpretations for the statistical methods referenced in the main text. Some metrics (e.g., Sharpe ratio, maximum drawdown) are reported in Section~\ref{sec:strategy}; others (e.g., ANOVA, eta-squared) are computed in the snapshot and referenced in Section~\ref{sec:results}. The definitions below serve as a reference for readers unfamiliar with these standard measures.
...
Factor returns for spanning tests (MKT, SMB, HML, RMW, CMA, MOM) are sourced from Kenneth French's data library, the academic standard for factor research. We run spanning tests at monthly frequency, aligning each monthly premium observation to the corresponding monthly factor returns (July through June) to match the July-June convention used in this study.
