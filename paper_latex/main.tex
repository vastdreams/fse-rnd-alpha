...
\author{%
    Abhishek Sehgal\,\orcidlink{0009-0000-9424-4695}\\
    \small FSE Research \& Investments Pty Ltd%
    \thanks{ABN: 35 688 556 747. Address: 50 Murray St, Pyrmont, Sydney, NSW 2009, Australia. Email: \href{mailto:abhishek@finsoeasy.com}{abhishek@finsoeasy.com}.}%
}
...
\noindent\textbf{Method.} Each year, we sort point-in-time S\&P 500 constituents into quintiles by R\&D intensity (R\&D expense divided by revenue) and evaluate subsequent July--June returns to mitigate look-ahead bias \citep{fama_french_1993}. Primary inference uses a non-overlapping annual high-minus-low premium series (Q5--Q1) spanning \AnnualSeriesStart{} to \AnnualSeriesEnd{} (\AnnualNYears{} observations). This timing convention aligns with Fama--French methodology to avoid look-ahead bias. Where historical constituent spans are available, we enforce point-in-time membership at formation dates; exits are handled via return construction (cash-after-exit) and delisting uncertainty is addressed via sensitivity analysis rather than a single hard-coded proxy.

\noindent\textbf{Results.} The high-minus-low premium (Q5 minus Q1) averages \textbf{\AnnualMeanPremium\%} per year in non-overlapping annual returns. In plain terms: stocks in the top 20\% by R\&D intensity outperformed the bottom 20\% by approximately \AnnualMeanPremium\% annually over the sample period. This difference is statistically significant with conservative Newey--West inference \citep{newey_west_1987} ($t = \AnnualTStat$, $p = \AnnualPValue$), and is positive in \AnnualPositiveYears{} of \AnnualNYears{} years (\AnnualWinRatePct\% win rate).

\noindent\textbf{Implementation.} For implementability, we map the signal into a simple long-only portfolio that holds the top \textbf{20} stocks by R\&D intensity (equal-weighted) and reconstitutes annually in July. Under a literature-calibrated transaction-cost model \citep{novy_marx_velikov_2016} using realized turnover from the investable backtest and S\&P 500 liquidity characteristics, estimated trading costs are \AnnualTradingCostPct\% per year, yielding a net premium of \NetPremiumAfterCostsPct\% after costs and a premium capture rate of \PremiumCaptureRatePct\%.
...
Throughout this paper, the \emph{premium} refers to the within-universe return spread between the high-R\&D and low-R\&D portfolios formed for the same period, typically the high-minus-low (HML) spread Q5 minus Q1. This is \textbf{not} automatically a benchmark excess return relative to the S\&P 500; benchmark comparisons are shown separately in the strategy section.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Premium (HML):} The return spread between high-R\&D and low-R\&D portfolios formed in the same universe for the same period. In most exhibits this is Q5 $-$ Q1 (highest R\&D quintile minus lowest R\&D quintile).
    ...
    \item \textbf{Not a benchmark excess return:} This premium is not automatically ``above the S\&P 500.'' Benchmark comparisons are shown separately in the investable strategy section (Section~\ref{sec:strategy}).
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Bias-aware timing:} The default return convention is July--June to mitigate look-ahead from filing lags \citep{fama_french_1993}. Most U.S.\ firms have December fiscal year ends and must file 10-K reports within 60--90 days. By waiting until July to form portfolios, we ensure all accounting data is publicly available. This is not a trivial choice: studies using calendar-year returns may inadvertently trade on private information.
    
    \item \textbf{Clear separation of inference vs.\ description:} Statistical inference is anchored on a non-overlapping annual premium series; overlapping rolling windows are used only for descriptive context. Annual non-overlapping observations reduce mechanical overlap and support cleaner inference. Rolling windows are autocorrelated (overlapping periods share data) but useful for visualizing trends and regime dependence. We are explicit about which is which to avoid the common pitfall of treating autocorrelated rolling-window statistics as independent evidence.
...
A recurring theme in the intangible-capital literature is that standard accounting can understate the economic value of R\&D by expensing it. \textbf{Why does this matter?} Because when R\&D is expensed immediately (rather than capitalized like physical assets), a firm investing heavily in innovation reports lower earnings today even if that investment will generate substantial future cash flows.
...
A competing interpretation is that high-R\&D firms load on innovation-related risks: uncertain payoffs, higher operating leverage, and sensitivity to funding conditions. \textbf{Why would investors demand a premium for these risks?} Because R\&D outcomes are inherently uncertain (most projects fail), high-R\&D firms tend to have more volatile cash flows, and innovation-heavy companies are more sensitive to economic downturns when funding dries up.
...
\begin{enumerate}[leftmargin=*]
    ...
    \emph{Why test this?} This is the fundamental question: does R\&D intensity predict returns? If there is no premium, the remaining hypotheses are moot.
    ...
    \emph{Why test this?} A premium that only worked in one decade (say, the tech bubble) would be less useful for forward-looking portfolios. We need to understand how the premium behaves across different market environments.
    ...
    \emph{Why test this?} If the premium disappears after controlling for sectors (e.g., just a tech bet) or standard factors (e.g., just a size effect), it would not represent a distinct signal. Factor spanning tests and double-sorts help isolate the R\&D-specific component.
    ...
    \emph{Why test this?} Academic premiums often disappear after trading costs. A premium that cannot be captured in practice is theoretically interesting but not actionable for investors. We need to show the premium is capturable after realistic implementation costs.
\end{enumerate}
...
This ratio captures how much a firm invests in research and development relative to its scale. \textbf{Why use revenue as the denominator?} Revenue is a stable, comparable measure of firm size that is less affected by capital structure or accounting choices than alternatives such as total assets or market capitalization. Assets can be inflated by acquisitions; market cap fluctuates with sentiment; revenue reflects operating scale.
...
\textbf{Why this timing matters:} Most U.S.\ firms have December fiscal year ends and must file 10-K reports within 60--90 days (by late February/March). By waiting until July to form portfolios, we ensure all accounting data is publicly available. Using calendar-year returns (January--December) would mean trading on data that wasn't yet public, inflating apparent performance. This is not a minor technical detail: look-ahead bias can create spurious premiums that disappear in real-time implementation.

The total shareholder return for each security includes dividends:
\begin{equation}
\text{TSR}_{i,t} = \frac{P_{i,t+1} + D_{i,t+1} - P_{i,t}}{P_{i,t}} \times 100\%
\end{equation}
where $P$ is price and $D$ is dividends paid during the period. In the Tier-1 pipeline we approximate TSR using provider-adjusted close (split+dividend adjusted per vendor) and do not add dividends separately (to avoid double counting).
...
We are explicit about which approach applies to each exhibit to avoid the common pitfall of treating autocorrelated statistics as independent evidence.
...
Table~\ref{tab:sample} reports sample construction and coverage statistics for the snapshot.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Universe:} S\&P 500 point-in-time constituents. Where historical membership spans are available, we include only stocks that were actually in the index at each formation date, reducing survivorship bias from excluding removed/failed companies. This is critical: a backtest using today's S\&P 500 membership would exclude companies that were members in the past but have since delisted, merged, or failed.
    ...
    \item \textbf{Inclusion:} Firms with R\&D reported as zero are retained (typically in Q1). A minimum-revenue filter is applied to avoid extreme ratios from very small denominators. Zero-R\&D firms are legitimate members of Q1---they simply don't invest in R\&D.
    
    \item \textbf{Exit handling:} When a stock's price history ends mid-period (merger/delisting), we compute the holding-period return to the last observed trading day and treat cash as earning 0\% thereafter for the remainder of the July--June window. We report delisting sensitivity as a separate robustness check.
\end{itemize}
...
\end{tabular}
\end{center}
The \textbf{HML premium} (High-Minus-Low) is Q5 return minus Q1 return:
\begin{equation}
\text{HML}_{t} = R_{Q5,t} - R_{Q1,t}
\end{equation}
...
\begin{equation}
d = \frac{\mu_{Q5} - \mu_{Q1}}{\sigma_{pooled}}
\end{equation}
...
\begin{tcolorbox}[title=Why Rolling Windows Show Lower Premiums at Longer Horizons, colback=yellow!5, colframe=yellow!40, breakable]
...
\begin{itemize}[leftmargin=*]
    \item A ``high R\&D'' firm in 2000 may reduce R\&D by 2010
...
\end{itemize}
\textbf{Bottom line:} The evidence supports H1 (existence of a positive characteristic premium).
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Dot-com bust (2000--2002):} Did the premium hold when tech stocks crashed?
    \item \textbf{Financial crisis (2008--2009):} How did high-R\&D firms perform during the credit crunch?
    \item \textbf{Post-2016 tech rally:} Is the recent premium inflated by a sector bubble?
\end{itemize}
...
\textbf{Why sector analysis matters:} If the R\&D premium is entirely driven by sector exposure (e.g., ``it's just a tech bet''), an investor could replicate it with a simpler sector allocation. The key question is: \emph{does the R\&D premium exist within sectors, or is it just a sector effect?}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Not a pure size/factor artifact:} Section~\ref{sec:robustness} reports factor spanning tests and size $\times$ R\&D double-sorts; we do not present a sector-neutralized premium in this snapshot.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Why does Medium sometimes differ?} Mid-cap stocks often behave idiosyncratically; the key comparison is Large vs.\ Small.
\end{itemize}
...
We report descriptive premiums across proxies for arbitrage costs (size, volatility, and coverage). These patterns can be suggestive for distinguishing mispricing from risk-based explanations, but they are not definitive.

\begin{itemize}[leftmargin=*]
    \item \textbf{Mispricing interpretation:} Premiums should be larger where arbitrage is more difficult (small size, high volatility, low analyst coverage). If sophisticated arbitrageurs cannot trade away the mispricing, it persists.
...
\begin{itemize}[leftmargin=*]
    \item If the premium is \textbf{larger in hard-to-arbitrage stocks} (small, volatile, low coverage), this is more consistent with mispricing.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Rebalancing:} Repeat annually. Sell stocks that leave Q5; buy stocks that enter Q5.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Net premium (benchmark-relative):} After costs, the strategy delivers \NetPremiumAfterCostsPct\% annual net premium versus the snapshot benchmark.
\end{itemize}
...
\addlegendentry{Benchmark (net)}
\addplot+[mark=none,thick] table [x=year,y=sp500_index,col sep=comma] {data/investable_growth.csv};
\addlegendentry{S\&P 500}
\end{axis}
\end{tikzpicture}
\caption{Investable strategy cumulative growth (snapshot backtest; incomplete terminal years removed). The R\&D strategy is a concentrated top-20 R\&D-intensity portfolio (equal-weighted) with annual reconstitution and transaction costs deducted. The benchmark is the snapshot-defined equal-weighted cohort universe; the S\&P 500 series is shown separately.}
\label{fig:investable_growth}
\end{figure}

\begin{tcolorbox}[title=Important Caveats for Backtests, colback=red!5, colframe=red!40]
\begin{itemize}[leftmargin=*]
...
The full codebase is available at: \url{https://github.com/vastdreams/rd-alpha-research}

Researchers can replicate this paper as follows:
\begin{enumerate}[leftmargin=*]
    \item Clone the repository:
    \begin{quote}
    \texttt{git clone }\url{https://github.com/vastdreams/rd-alpha-research.git}
    \end{quote}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Implementability:} After transaction costs, the strategy retains \PremiumCaptureRatePct\% of the gross premium, delivering \NetPremiumAfterCostsPct\% annual net premium.
\end{enumerate}
...
\url{https://github.com/vastdreams/rd-alpha-research}\\[0.5em]
...
Factor returns for spanning tests (MKT, SMB, HML, RMW, CMA, MOM) are sourced from Kenneth French's data library, the academic standard for factor research. These factors are updated monthly and widely used in peer-reviewed research.
