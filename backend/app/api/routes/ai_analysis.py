"""
PATH: backend/app/api/routes/ai_analysis.py
PURPOSE:
  - AI-powered R&D analysis endpoints using DeepSeek 3.2
  - Concurrent analysis for research insights
"""
...
async def get_company_rd_data(session: AsyncSession, symbol: str) -> dict:
    ...
    # Get income statements with R&D
    ...
    return {
        ...
        "avg_rd_intensity": avg_intensity,
        "years_with_rd": len(rd_by_year),
...
async def analyze_sector(
    ...
    # Get all companies in sector with R&D data
...
async def generate_research_insights(
    ...
    trends_result = await session.execute(text("""
        ...
        WHERE rd_expenses > 0 AND revenue > 0
    ...
    async with DeepSeekClient(DEEPSEEK_API_KEY) as client:
        insights = await client.generate_research_insights(
            rd_trends, sector_comparison, top_performers
        )
    
    return {
        "data_summary": {
            "years_analyzed": len(rd_trends),
            "sectors_analyzed": len(sector_comparison),
...
async def get_cohort_recommendations(
    ...
    # Get companies with strong R&D profiles
    result = await session.execute(text("""
        WITH rd_stats AS (
            SELECT 
                i.symbol,
        ...
        SELECT * FROM rd_stats
        ORDER BY 
            years_of_data DESC,
            avg_rd_intensity DESC
        LIMIT :limit
    """), {"limit": target_size * 2})
    
    candidates = [dict(r._mapping) for r in result.fetchall()]
    
    # Use AI to recommend optimal cohort
...
- intensity_distribution: dict with high/medium/low counts"""
    ...
    return {
        ...
        "all_candidates": candidates[:target_size]
    }
