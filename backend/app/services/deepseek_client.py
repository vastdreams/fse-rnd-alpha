# DeepSeek API client for AI-powered R&D analysis (V3.2 model, concurrent support).
import asyncio
import aiohttp
import logging
import json
...
class DeepSeekClient:
    """Async client for DeepSeek API with concurrent analysis support (V3.2 Speciale)."""
        ...
        if response:
            try:
                ...
                return AnalysisResult(
                    symbol=symbol, analysis_type="rd_profile",
                    content=content, confidence=0.85, model=self.model
                )
            except:
                return AnalysisResult(
                    symbol=symbol, analysis_type="rd_profile",
                    content={"raw_analysis": response}, confidence=0.7, model=self.model
                )
        return AnalysisResult(
            symbol=symbol, analysis_type="rd_profile",
            content={"error": "Analysis failed"}, confidence=0.0, model=self.model
        )
...
Format as JSON with clear structure."""
    ...
    async def batch_analyze_companies(
        ...
        """Concurrently analyze multiple companies. Returns List[AnalysisResult]."""
    ...
    def _extract_json(self, text: str) -> Dict[str, Any]:
        """Extract JSON from response text (tries direct parse, code block, then any JSON structure)."""
        try:
